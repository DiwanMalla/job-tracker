...
export async function GET() {
  try {
    ...
    const settings = await ShareService.getShareSettings(session.user.id);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const settings = await ShareService.createOrUpdateShareSettings(
      session.user.id,
...
export async function DELETE() {
  try {
    ...
    await ShareService.deleteShareSettings(session.user.id);
