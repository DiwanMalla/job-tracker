      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">{application.position}</h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-200">
            <div>
              ...
              <p className="text-sm font-medium text-gray-900">{formatDate(application.applicationDate)}</p>
            </div>
            ...
            {application.salary && (
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{formatCurrency(application.salary)}</p>
              </div>
            )}
            {application.followUpDate && (
              <div>
                <p className="text-sm text-gray-500">Follow Up</p>
                <p className="text-sm font-medium text-gray-900">{formatDate(application.followUpDate)}</p>
              </div>
            )}
          </div>
            ...
            <div className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {application.resume ? (
                    <p className="text-xs text-gray-500 mt-1">{application.resume.originalName}</p>
                  ) : (
                    <p className="text-xs text-gray-500 mt-1">No resume uploaded</p>
                  )}
                </div>
                {application.resume && (
                  <Link
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {application.coverLetter ? (
                    <p className="text-xs text-gray-500 mt-1">{application.coverLetter.originalName}</p>
                  ) : (
                    <p className="text-xs text-gray-500 mt-1">No cover letter uploaded</p>
                  )}
                </div>
                {application.coverLetter && (
                  <Link
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row gap-4 justify-between">
            ...
            <DeleteApplicationButton applicationId={application.id} companyName={application.companyName} />
          </div>
        </div>
      </div>
    </DashboardLayout>
