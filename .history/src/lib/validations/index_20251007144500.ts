...
export const paginationSchema = z.object({
  page: z.string().or(z.number()).transform((val) => parseInt(val.toString(), 10)).pipe(z.number().positive().default(1)),
  limit: z.string().or(z.number()).transform((val) => parseInt(val.toString(), 10)).pipe(z.number().positive().max(100).default(10)),
});
